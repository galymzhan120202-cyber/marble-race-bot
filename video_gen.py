...
STRONG_HASHTAG_POOL = [
    "#shorts", "#mazerace", "#marblerace", "#satisfying", "#race",
    "#physics", "#fyp", "#viral", "#whowins", "#simulation", "#labyrinth",
]

TITLE_TEMPLATES = [
    "{names} — Maze Race! Who Wins?",
...
MUSIC_QUERIES = [
    "upbeat", "electronic beat", "fun", "energetic", "playful",
    "chiptune", "adrenaline", "arcade", "bouncy", "cinematic action",
]
...
def _try_fetch_openverse_music(query, min_duration_sec):
    response = requests.get(
        ...
        timeout=15,
        headers={"User-Agent": "MazeRaceBot/1.0 (automated background music fetch)"}
    )
...
_ALL_RACER_NAMES = [r["name"] for r in RACER_POOL]


def _racer_set_from_title(title):
    found = set()
    for name in _ALL_RACER_NAMES:
...
def get_recent_matchups(max_results=12):
    """Returns the set of racer rosters (as frozensets of names) seen in the
    channel's most recent uploads, so a freshly generated race can be
    checked against them and re-rolled if it's an exact repeat."""
    titles = get_recent_channel_titles(max_results)
    matchups = [_racer_set_from_title(t) for t in titles]
    ...
    try:
        ...
        request_body = {
            "snippet": {
                ...
                "tags": tags or ["maze race", "marble race", "shorts"]
            },
            "status": {
...
def pick_background_music(duration):
    """Shared by the Shorts pipeline and the Tournament long-form pipeline:
    tries Openverse first, falls back to a local file in music/. Returns
    (path_or_None, attribution_or_None)."""
    music_path = None
    ...
    return music_path, music_attribution
...
def generate_video(skip_upload: bool = False, n_racers: int = None):
    try:
        logger.info("🎬 Maze Race видео құру процессі басталды")
        ...
        try:
            generate_thumbnail(race, thumbnail_path)
        ...
        try:
            race_clip = build_race_clip(race)
            duration = race_clip.duration
